...
<div class="site-calendar">
    ...
    <div class="index-section double-index-section">
        <div class="flex-div" style="align-self: flex-start;">
            ...
            <?= Html::button('Modific informațiile curente', ['class' => 'btn btn-primary', 'style' => 'min-width: 40%; margin-bottom: 1em;', 'id' => 'set-edit-concert']) ?>
            <?= Html::button('Creez concert nou', ['class' => 'btn btn-primary', 'style' => 'min-width: 40%; margin-bottom: 1em;', 'id' => 'set-create-concert']) ?>

            <div id="edit-concert">
                <?php
                    $form = ActiveForm::begin([
                        'id' => 'concert-edit-date-form',
                ...
                <?= $form->errorSummary($currentConcertModel);?>
    
                <?= $form->field($currentConcertModel, 'title')->label(Yii::t('app', 'Titlu')) ?>
    
                <?= $form->field($currentConcertModel, 'description')->textarea(['rows' => 2, 'style' => 'min-height: 80px; overflow: auto;'])->label(Yii::t('app', 'Descrierea concertului')) ?>
    
                <?= $form->field($currentConcertModel, 'date')->widget(DateTimePicker::class, [
                ...
                <br>
                <?= Html::submitButton('Salvează Modificările', ['class' => 'btn btn-success', 'id' => 'edit-date-button']) ?>
    
                <?php ActiveForm::end(); ?> 
            </div>
            <div id="create-concert" style="display: none;">
                <?php
                    $form = ActiveForm::begin([
                ...
                <?= $form->field($concertModel, 'date')->widget(DateTimePicker::class, [
                    ...
                    'value' => $concert_date,
                    'type' => DateTimePicker::TYPE_COMPONENT_PREPEND,
                    'options' => ['id' => 'create-concert-date', 'placeholder' => $concert_date],
                    'pluginOptions' => [
                        'autoclose' => true,
                        'format' => 'yyyy-mm-dd hh:ii',
                        'todayHighlight' => false,
...
$js = <<<JS
    ...
    edit_div = document.getElementById('edit-concert');
    create_div = document.getElementById('create-concert');
    document.getElementById('set-edit-concert').addEventListener('click', function() {
        edit_div.style = '';
        create_div.style = 'display: none;'
    });
    document.getElementById('set-create-concert').addEventListener('click', function() {
